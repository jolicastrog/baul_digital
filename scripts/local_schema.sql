-- ============================================================================
-- BAÚL DIGITAL - ESQUEMA PARA DESARROLLO LOCAL (PostgreSQL)
-- ============================================================================
-- Este archivo está adaptado para PostgreSQL local sin Supabase Auth
-- Para producción con Supabase, usar el archivo original con RLS completo
-- ============================================================================
...
CROSS JOIN (
  VALUES
    ('Identificación', 'Cédulas, pasaportes, licencias', 'User', '#1e40af', 1),
    ('Financieros', 'Estados de cuenta, contratos bancarios', 'CreditCard', '#059669', 2),
    ('Salud', 'Seguros médicos, exámenes', 'Heart', '#dc2626', 3),
    ('Educación', 'Certificados, diplomas', 'GraduationCap', '#7c3aed', 4),
    ('Laboral', 'Contratos, certificados laborales', 'Briefcase', '#ea580c', 5)
...
CROSS JOIN (
  VALUES
    ('cedula.pdf', 245760, 'application/pdf', '1234567890/identificacion/cedula.pdf', 'Cédula de ciudadanía', '2026-12-31'::DATE, ARRAY['oficial', 'identidad']),
    ('pasaporte.pdf', 512000, 'application/pdf', '1234567890/identificacion/pasaporte.pdf', 'Pasaporte ecuatoriano', '2027-06-15'::DATE, ARRAY['viajes', 'internacional'])
) AS doc(file_name, file_size_bytes, file_type, storage_path, description, expiry_date, tags)
WHERE p.cedula_unica = '1234567890'
  AND c.name = 'Identificación'

...
const SECTIONS = [
  ...
  },
  {
    ...
    title: 'Categorías y búsqueda',
    intro: 'Organiza tus documentos en grupos y encuéntralos rápidamente.',
    steps: [
      { label: 'Asignar categoría al subir', desc: 'Al subir un archivo, selecciona la categoría en el campo del formulario de carga. Puedes dejarlo vacío y asignarlo después.' },
      { label: 'Mover un documento a otra categoría', desc: 'En la lista de documentos, haz clic en el botón de carpeta con flecha (→📂) a la derecha de cada archivo. Se abre un menú con tus categorías disponibles: selecciona la destino. También puedes elegir "Sin categoría" para quitar la clasificación actual.' },
      { label: 'Filtrar por categoría', desc: 'Encima de la lista de documentos aparecen botones en forma de etiqueta, uno por cada categoría. Haz clic en uno para ver solo esos archivos, o en "Todos los Archivos" para quitar el filtro.' },
      { label: 'Buscar documentos por nombre', desc: 'Usa la barra de búsqueda (lupa 🔍) en la parte superior de tu bóveda para encontrar documentos por nombre, sin importar en qué categoría estén. La búsqueda y el filtro de categoría se pueden usar juntos.' },
    ],
    notes: [
      { text: 'El plan Gratuito incluye 6 categorías predefinidas (Identidad, Vehículo, Salud, Educación, Finanzas, Otros).', premium: false },
      { text: 'Plan Premium: crea hasta 25 categorías personalizadas con el nombre que prefieras.', premium: true },
  ...
  },
  {
    ...
    steps: [
      { label: 'Abrir la vista previa', desc: 'En la lista de documentos, haz clic en el botón "Ver" (ícono de ojo) del archivo que quieras consultar. El sistema obtiene un enlace seguro y abre el visor.' },
      { label: 'Qué muestra el visor según el tipo de archivo', desc: 'PDF → visor con barra de herramientas completa (zoom, navegación de páginas). Imágenes (JPEG, PNG, WebP) → visualización a tamaño completo. Audio MP3 → reproductor de audio integrado. Video MP4 → reproductor de video integrado. Word y Excel → se abren en una nueva pestaña del navegador para descargar y abrir con tu app de oficina.' },
      { label: 'Descargar desde el visor', desc: 'Dentro del visor, haz clic en el botón "Descargar" (ícono ↓) ubicado en la barra superior derecha para guardar el archivo en tu dispositivo.' },
      { label: 'Cerrar el visor', desc: 'Haz clic en la "X" en la esquina superior derecha o presiona Escape para cerrar el visor y regresar a tu bóveda.' },
    ],
    notes: [
      { text: 'Los enlaces de acceso expiran automáticamente cada 15 minutos por seguridad. Si el archivo no carga, cierra el visor y vuelve a abrirlo.', premium: false },
      { text: 'Vista previa integrada disponible para: PDF, imágenes (JPEG, PNG, WebP), audio MP3 y video MP4.', premium: false },
      { text: 'Para Word (.doc/.docx) y Excel (.xls/.xlsx), el botón "Ver" abre una pestaña de descarga — ábrelos con Microsoft Office, LibreOffice o Google Docs.', premium: false },
    ],
    limits: null,
  },
  {
